...
``` {r 5 min interval varaible, echo=TRUE}
library(dplyr)
activity <- activity %>%
...
``` {r HM Formatting Function, echo=TRUE}
HM <- function(x) {
        h <- floor(x/60)
        m <- floor(x %% 60)
...
```{r Question 1, echo=TRUE, fig.width=5}
library(dplyr)
activity$date <- as.Date(as.character(activity$date), "%Y-%m-%d")
...
```{r Question 2, echo=TRUE, fig.width=6}
...
```{r Question 3, echo=TRUE , fig.width=5}
...
The total number of NAs are `r total_NAs`  

The **mean** for the total number of steps taken per day  
without NA values = **`r mean`**    
imputing NA values = **`r mean1`**    

The **median** for the total number of steps taken per day  
without NA values = **`r median`**    
imputing NA values = **`r median1`**    
...
```{r Question 4, echo=TRUE, fig.width=6}
activity$date <- as.Date(as.character(activity$date), "%Y-%m-%d")
